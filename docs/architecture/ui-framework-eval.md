...
- Android build/install proof passed on a Pixel 7 Pro over wireless ADB. Required fixes: enable Slint's `backend-android-activity-06`, expose `android_main` from a `cdylib`, add cargo-apk manifest metadata, set Android SDK/NDK env explicitly, and use `strip = "strip"` to reduce the debug APK from 442M to 89M. Validation passed: `cargo check -p slint-poc --target aarch64-linux-android`, `cargo-apk apk build -p slint-poc --target aarch64-linux-android --lib`, `adb install --no-incremental -r`, `adb shell pidof com.anotherone.slintpoc`, and focused-window verification via `dumpsys window`.
