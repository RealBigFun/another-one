...
- Left navigation: one `PROJECTS` tree. Root project rows own their nested task/worktree rows; task rows are not a sibling global section. Project row activation opens project overview state, while task row activation attaches a terminal section.
...
- `AoIconButton`, `AoSplitButton`, `AoStatusPill`, `AoResourceIndicator`, `AoSidebarProjectTreeRow`, `AoSidebarTaskTreeRow`, `AoTabChip`, `AoStateCard`, `AoToast`.
