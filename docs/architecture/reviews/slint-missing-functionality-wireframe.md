...
| Project row menu | `desktop/src/left_sidebar.rs`, `desktop/src/app.rs` | Anchored menu exists but uses fixture entries. | Open project, open GitHub, Open In, remove project, show destructive confirmation when needed. | Partial: Open In and GitHub are ready; `RemoveProject` adapter is missing in `desktop/src/daemon_host.rs`. | `another-one-dn3` |
| Task row menu | `desktop/src/left_sidebar.rs`, `desktop/src/app.rs` | Anchored menu exists but uses fixture entries and placeholder rename/delete behavior. | Select/open task, rename task, pin/unpin task, remove task/worktree, preserve row identity and active section. | Partial: protocol has `RenameTask`, `SetTaskPinned`, `RemoveTask`; `desktop/src/daemon_host.rs` does not implement the registry adapter yet. | `another-one-dn3` |
