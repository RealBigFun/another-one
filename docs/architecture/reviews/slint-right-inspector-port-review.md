...
- Current slice implements the real toolbar, Changes loading/clean/dirty/error/unavailable states, file/group action callbacks, destructive discard confirmation, Compare-mode availability/data loading, and daemon snapshot refresh.
- Deferred to later loops: commit row expansion/file-change details, full Checks rows, collapse persistence, pending action spinners, and capture parity.
