        ...
        let row = div()
            ...
            .on_mouse_down(
                ...
                cx.listener(move |this, _ev: &MouseDownEvent, window, cx| {
                    ...
                    // Narrow (phone) layout: tapping a project pushes the
                    // workspace onto the master/detail nav stack so the
                    // sidebar gets out of the way. Wide-mode behavior is
                    // unchanged — sidebar stays visible alongside.
                    if this.is_narrow(window) {
                        this.mobile_push(MobileView::Project(pid_row.clone()), cx);
                    }
                }),
            );
