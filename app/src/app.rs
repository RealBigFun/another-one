...
impl AnotherOneApp {
    ...
    pub(crate) fn sync_registry_project_store(&self) {
        if let Ok(mut state) = self.registry_state.lock() {
            ...
            // Fire the broadcast tick so any connected session's
            // push pump sends a fresh `WorkerReply::ProjectList`.
            // Routes through `RegistryState::notify_state_changed`
            // so future instrumentation has one place to land —
            // see #136.
            state.notify_state_changed();
        }
    }
