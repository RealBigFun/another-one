...
impl BuildIdentity {
    ...
    pub fn auto_polling_enabled(&self) -> bool {
        // No point checking for updates if we can't verify the
        // signature. Without a baked-in `ANOTHER_ONE_UPDATE_TRUST_PUBKEY_HEX`
        // every check would fail with "trust pubkey unset" and toast
        // the user every poll interval. Local-dev builds typically
        // omit the key on purpose — silently skip polling rather than
        // spamming toasts.
        if TRUST_PUBKEY_HEX.is_none() {
            return false;
        }
