        ...
        match e.kind() {
            ...
            _ => {
                ...
                if let Some(libc::EIO) | Some(libc::EINVAL) = e.raw_os_error() {
                    ...
                    // Some kernels (notably Android, and some older Linux
                    // drivers / virtio-net NICs) accept `UDP_SEGMENT` at
                    // `setsockopt` time but then fail the actual `sendmsg`
                    // with `EIO` (or `EINVAL`). Detect that here by seeing
                    // that this transmit had GSO set, and retry it split
                    // into individual datagrams with no `UDP_SEGMENT` cmsg.
                    // See quinn-rs/quinn#2399.
                    if transmit.effective_segment_size().is_some() {
                        ...
                        for chunk in transmit.contents.chunks(segment_size) {
                            ...
                            loop {
                                let n = unsafe {
                                    libc::sendmsg(io.as_raw_fd(), &msg_hdr, 0)
                                };
                                if n >= 0 {
                                    break;
                                }
