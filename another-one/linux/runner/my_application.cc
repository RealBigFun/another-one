...
static void my_application_activate(GApplication* application) {
  ...
  // for parity with the GPUI build. `gtk_window_set_decorated(FALSE)`
  // drops the GTK title bar + min/max/close stack on X11; under
  // Wayland with client-side decorations this also suppresses the
  // CSD bar.
  gtk_window_set_title(window, "AnotherOne");
