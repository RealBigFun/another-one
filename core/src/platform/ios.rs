...
impl HeadlessPlatform for IosPlatform {
    ...
    fn open_external_url(_url: &str) -> Result<(), String> {
        ...
        // which is reachable from Swift/Objective-C only. This
        // Rust-side implementation exists only so the trait shape is
        // the same on every target; platform UI code should handle it.
        Err("open_external_url not supported from Rust on iOS; use platform UI code".into())
    }
    ...
    fn is_open_in_app_available(_app: OpenInAppKind) -> bool {
        ...
        // accessible to a sandboxed Rust library; platform UI code
        // must talk directly to UIKit.
        false
    }
