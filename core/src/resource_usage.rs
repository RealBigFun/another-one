//! Hierarchical resource-usage sampler — port of GPUI desktop's
//! `desktop/src/resource_usage.rs::ResourceUsageSampler`.
//!
//! Pure data + aggregation. The platform-specific PID walker lives in
//! `core::platform::CurrentPlatform::read_process_samples`; this module
//! deltas the cumulative-CPU-time samples between ticks and groups
//! them into the project → task → session tree the resource indicator
//! popover renders.
...
#[derive(Clone, Debug, Default, PartialEq)]
pub struct ResourceUsageProject {
    pub key: String,
    pub label: String,
    pub cpu_percent: f32,
    pub memory_bytes: u64,
...
#[derive(Clone, Debug, Default, PartialEq)]
pub struct ResourceUsageSession {
    pub key: String,
    pub label: String,
