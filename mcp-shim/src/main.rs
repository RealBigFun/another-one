...
fn run(socket_path: &Path) -> io::Result<()> {
    ...
    // stdin → socket
    let stdin_thread = thread::spawn(move || copy_until_eof(io::stdin().lock(), stream_write));

    // socket → stdout. Runs on the main thread so main exits
    // when the server closes the socket.
    let stdout = io::stdout();
    copy_until_eof(stream_read, stdout.lock())?;

    // Upstream closed — try to reap the stdin thread quickly.
    // If the harness hasn't closed stdin yet, the thread will
    // be blocked on a read; its join blocks until the process
    // exits, which is fine (the OS cleans up).
